...
def commit_to_diff(codeLink):
    try:
        codeSoup=BeautifulSoup(urlopen(Request(codeLink,
                              ...
                              'html.parser')
        #meta data div
        codeDiv = codeSoup.find('span', class_='Truncate')
        for span in codeDiv.findAll('span'):
            a = span.find('a')
            if span.get_text() == "[diff]":
                diffLinks = "https://android.googlesource.com"+a['href']
...
if __name__ == "__main__":
    data_csv = pd.read_csv("./csv/owasp_data_oneLink.csv")
    data_csv["commit_message"] = None
    data_csv["diff_message"] = None
    data_csv["diff_unified"] = None
    data_csv["project_before"] = None
    data_csv["project_after"] = None
    for index, row in data_csv.iterrows():
        ...
        if not diffLink=="":
            ...
            data_csv.loc[index, "commit_message"] = commit_message
            # print(commit_message)
            # print("------------------")
            data_csv.loc[index, "diff_message"] = diff_message
            # print(diff_message)
            # print("------------------")
            data_csv.loc[index, "diff_unified"] = diff_unified
            # print(diff_unified)
            # print("------------------")
            data_csv.loc[index, "project_before"] = before
            # print(before)
            # print("------------------")
            data_csv.loc[index, "project_after"] = after
            # print(after)
            # print("------------------")
            print("done", index)
    data_csv.to_csv("./csv/cve_commit.csv")
    print("done")